...
export class MobileMenuToggle {
    ...
    show() {
        ...
        if ($(window).width() > 1024) {
            var position = this.$header.position(), height = this.$header.outerHeight();
            $('.themevale_Mobile.themevale_MobileMenu').css({'top': (position.top + height), 'height': ($(window).height() - position.top - height) });
        } else {
