...
import GalleryPopup from '../themevale/themevale_GalleryPopup';
import pdpAddonUpsell from '../custom/addon-upsell';
...
export default class ProductDetails {
    constructor($scope, context, productAttributesData = {}) {
        ...
        $(document).on('click', '#form-action-addToCart-2', function (event) {
            $form2.submit();
        });
    ...
    getViewModel($scope) {
        return {
            ...
            $addToCart2: $('#form-action-addToCart-2', $scope),
    ...
    addProductToCart(event, form) {
        ...
        const mainFormData = this.filterEmptyFilesFromForm(new FormData(form));

        // helper: original main add-to-cart logic
        const addMainProduct = () => {
            console.log('adding main product');
            utils.api.cart.itemAdd(mainFormData, (err, response) => {
        ...
        // ---- UPSell hook here ----
        const $addonCheckbox = $('#addon-upsell-input');
        const shouldAddAddon =
            $('#addon-upsell').is(':visible') &&
            $addonCheckbox.length &&
            $addonCheckbox.is(':checked');

        if (!shouldAddAddon) {
            // no upsell → normal behavior
            return addMainProduct();
        }

        // add addon first
        const addonData = new FormData();
        addonData.append('action', 'add');
        addonData.append('product_id', 6561); // 👈 your upsell product ID
        addonData.append('qty[]', '1');

        console.log('adding addon first');
        utils.api.cart.itemAdd(addonData, (addonErr, addonRes) => {
            const addonError = addonErr || (addonRes && addonRes.data && addonRes.data.error);

            if (addonError) {
                console.error('Error adding addon product:', addonError);
                // choice: still let main product be added even if addon fails
                // if you want to BLOCK in that case, uncomment next lines:
                // $addToCartBtn.val(originalBtnVal).prop('disabled', false);
                // this.$overlay.hide();
                // return showAlertModal('Could not add the add-on item. Please try again.');

                // fall through and still add main product:
            }

            // now add the main product with original logic
            addMainProduct();
        });
    }
    ...
    productImage_Mobile() {
        ...
        $(window).resize(function () {
            if ($(window).width() <= 1024) {
                if ($('.productView-image-mobile').length) {
                    if (!$('.productView-image-mobile').hasClass('slick-slider')) {
                        $('.productView-image-mobile').slick({
                            dots: true,
                            arrows: false,
                            infinite: true,
