...
export default function (context) {
    ...
    function productOptions() {
        ...
        $(document)
            .off('change.fbtOptions')
            .on('change.fbtOptions', '[data-fbt-option-change]', event => {
                productOptionsChanged(event);
            });

        // CLOSE when clicking the X
        $(document)
            .off('click.fbtOptionsClose')
            .on('click.fbtOptionsClose', '.close-options', function (e) {
                e.preventDefault();
                const $panel = $(this).closest('.themvale-fbt-detail-options');
                $panel.slideUp();
            });

        // CLICK ANYWHERE OUTSIDE → CLOSE ALL FBT OPTION PANELS
        $(document)
            .off('click.fbtOptionsOutside')
            .on('click.fbtOptionsOutside', function (e) {
                const $target = $(e.target);

                // if click is inside an options panel or on its toggle, do nothing
                if (
                    $target.closest('.themvale-fbt-detail-options').length ||
                    $target.closest('.themvale-fbt-toggle-options').length
                ) {
                    return;
                }

                // otherwise close all open FBT option panels
                $('.themvale-fbt-detail-options:visible').slideUp();
            });
    }
